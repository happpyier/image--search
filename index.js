...
app.get('/:id', function(request, response) {
  ...
  var API_KEY = secretKey; // specify your API key here
	plus.people.get({ auth: API_KEY, userId: '+RyuuLavitz' }, function(err, user) {
	  response.send('User<br/>'+Object.keys(user.image)+'<br/>Result: ' + (err ? err.message : user.displayName) + '<br/>Image:<br/><img style="width:304px;height:228px;" src='+user.image.url+'>');
	});
